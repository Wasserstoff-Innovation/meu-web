...
const Ob1 = () => {
  ...
  const handleNext = () => {
    ...
    if (data.name.trim().length === 0 || data.name.length < 2 || data.name.length > 50) {
    ...
    if ((data.mobile.countryCode.trim() || data.mobile.mobileNumber.trim()) && !mobilePattern.test(data.mobile.countryCode + data.mobile.mobileNumber)) {
    ...
    if (hasError) {
      ...
      const updatedUserData = { ...userData, name: data.name, email: data.email, mobile: { ...data.mobile }, location: { ...data.location } };
  ...
  const handleLocationChange = (address: any) => {
    ...
    if (
     ...
     console.error("Incomplete location object:", address);
    }
  };
  ...
  return (
    <div className="flex flex-1 flex-col justify-between items-end gap-4 py-8 ">
      ...
      <div className="w-full flex justify-center gap-5">
        <Button className="text-sm center" color="default" isDisabled={Boolean(userData.linkedin.email)} onClick={() => { window.location.href = getLinkedinOAuthUrl(); }}>
          <img src="/linkedin.svg" alt="Linkedin" className="h-6 w-6" /> LinkedIn
        </Button>
        <Button className="text-sm center" color="default" isDisabled={Boolean(userData.twitter.id)} onClick={() => { window.location.href = getTwitterOAuthUrl(); }}>
          <img src="/x.svg" alt="X" className="h-6 w-6" /> X (Twitter)
        </Button>
      </div>
      <div className="w-full">
        <p className="text-white text-sm mb-1">Full Name </p>
        <Input name="name" type="text" placeholder="John Doe" isClearable onChange={handleChange} value={data.name} />
        {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name}</p>}
        <p className="text-white text-xs mt-1">Your name can have 2-50 characters.</p>
      </div>
      <div className="w-full">
        <p className="text-white text-sm mb-1">Email </p>
        <Input name="email" type="email" placeholder="johndoe@domain.com" isClearable onChange={handleChange} value={data.email} />
        {errors.email && <p className="text-red-500 text-xs mt-1">{errors.email}</p>}
        <p className="text-white text-xs mt-1">Please enter your email address.</p>
      </div>
      <div className="w-full">
        ...
        <p className="text-white text-xs mt-1">Please enter your mobile number.</p>
      </div>
      <div className="w-full">
      ...
      <div className="w-full">
        <p className="text-white text-xs mt-1 ">By continuing, you agree to our <a href="#" className="text-primary-300">Terms of Service</a> and <a href="#" className="text-primary-300">Privacy Policy</a>.</p>
      </div>
      <div className="flex w-full justify-end">
        <Button className="mt-2 text-sm " color="primary" size="lg" onClick={handleNext}>Continue</Button>
      </div>
    </div>
  );
};
